...
# from celery.app import control, task
...
from nonovium_video_backend.actions.models import USER_MEDIA_ACTIONS, MediaAction
...
@celery_app.task(
    name="chunkize_media", bind=True, queue="short_tasks", soft_time_limit=60 * 30
)
...
@celery_app.task(name="produce_sprite_from_video", queue="long_tasks")
def produce_sprite_from_video(friendly_token):
    ...
    with tempfile.TemporaryDirectory(dir=settings.TEMP_DIRECTORY) as tmpdirname:
        try:
            ...
            cmd = "{0} -i {1} -f image2 -vf z'fps=1/10, scale=160:90' {2}&&files=$(ls {3}/img*.jpg | sort -t '-' -n -k 2 | tr '\n' ' ')&&convert $files -append {4}".format(  # noqa
                settings.FFMPEG_COMMAND,
                media.media_file.path,
                tmpdir_image_files,
                tmpdirname,
...
@celery_app.task(name="create_hls", queue="long_tasks")
...
@celery_app.task(name="check_running_states", queue="short_tasks")
...
@celery_app.task(name="check_media_states", queue="short_tasks")
...
@celery_app.task(name="check_pending_states", queue="short_tasks")
...
@celery_app.task(name="check_missing_profiles", queue="short_tasks")
...
@celery_app.task(name="clear_sessions", queue="short_tasks")
def clear_sessions():
...
@celery_app.task(name="save_user_action", queue="short_tasks")
...
@celery_app.task(name="get_list_of_popular_media", queue="long_tasks")
...
@celery_app.task(name="update_listings_thumbnails", queue="long_tasks")
...
@celery_app.task(name="remove_media_file", base=Task, queue="long_tasks")
