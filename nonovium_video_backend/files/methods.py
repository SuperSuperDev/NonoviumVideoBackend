...
from nonovium_video_backend.cms import celery_app, settings
...
def pre_save_action(media, user, session_key, action, remote_ip):
    ...
    from nonovium_video_backend.actions.models import MediaAction
...
def is_nonoviumvideo_editor(user):
    """Whether user is NonoviumVideo editor"""
...
def is_nonoviumvideo_manager(user):
    """Whether user is NonoviumVideo manager"""
...
def get_next_state(user, current_state, next_state):
    ...
    if is_nonoviumvideo_editor(user):
