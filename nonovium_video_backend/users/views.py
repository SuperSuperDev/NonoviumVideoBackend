...
from nonovium_video_backend.cms.permissions import IsUserOrManager
from nonovium_video_backend.files.management_views import UserSerializer
from nonovium_video_backend.files.methods import (
    is_nonoviumvideo_editor,
    is_nonoviumvideo_manager,
)
from nonovium_video_backend.users.forms import ChannelForm
from nonovium_video_backend.users.serializers import (
    LoginSerializer,
...
def view_user(request, username):
    ...
    context["CAN_EDIT"] = (
        True
        if ((user and user == request.user) or is_nonoviumvideo_manager(request.user))
        else False
    )
    context["CAN_DELETE"] = True if is_nonoviumvideo_manager(request.user) else False
    context["SHOW_CONTACT_FORM"] = (
        True if (user.allow_contact or is_nonoviumvideo_editor(request.user)) else False
    )
...
def edit_user(request, username):
    user = get_user(username=username)
    if not user or (
        user != request.user and not is_nonoviumvideo_manager(request.user)
    ):
...
def contact_user(request, username):
    ...
    if user and (user.allow_contact or is_nonoviumvideo_editor(request.user)):
...
class UserDetail(APIView):
    ...
    @swagger_auto_schema(
        ...
        responses={
            201: openapi.Response("response description", UserDetailSerializer),
            400: "bad request",
        },
    )
...
class UserWhoami(generics.RetrieveAPIView):
    ...
    @swagger_auto_schema(
        ...
        responses={
            200: openapi.Response("response description", UserDetailSerializer),
            403: "Forbidden",
        },
    )
...
class LoginView(APIView):
    ...
    @swagger_auto_schema(
        ...
        operation_description="Login url endpoint. According to what the portal provides, you may provide username and/or email, plus the password",  # noqa
        manual_parameters=[
            openapi.Parameter(
                name="username",
                in_=openapi.IN_FORM,
        ...
        responses={
            200: openapi.Response("user details", LoginSerializer),
            404: "Bad request",
        },
    )
